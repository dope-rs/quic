...
fn padded_payload() -> Vec<u8> {
    let padding_len = TARGET_LEN - PLAIN_HEADER.len() - PAYLOAD.len() - TAG_LEN;
    let mut v = PAYLOAD.to_vec();
    v.extend(std::iter::repeat_n(0u8, padding_len));
    v
}

#[test]
fn rfc9001_a2_client_initial_encrypts_to_protected() {
    let secrets = InitialSecrets::from_dcid(&DCID);
    let keys = PacketKeys::aes_128(&secrets.client);
    let prot = PacketProtection::aes_128(&keys);

    let payload = padded_payload();
    let out = prot.encrypt_long(&PLAIN_HEADER, &payload, PACKET_NUMBER, PN_OFFSET, PN_LEN);
    assert_eq!(out.len(), TARGET_LEN);
    assert_eq!(out.as_slice(), PROTECTED);
}

#[test]
fn rfc9001_a2_round_trip_via_server_keys() {
    let secrets = InitialSecrets::from_dcid(&DCID);
    let client_prot = PacketProtection::aes_128(&PacketKeys::aes_128(&secrets.client));

    let payload = padded_payload();
    let mut wire =
        client_prot.encrypt_long(&PLAIN_HEADER, &payload, PACKET_NUMBER, PN_OFFSET, PN_LEN);

    let server_prot = PacketProtection::aes_128(&PacketKeys::aes_128(&secrets.client));
    let recovered = server_prot.decrypt_long(&mut wire, PN_OFFSET).unwrap();
...
fn decrypt_rejects_tampered_tag() {
    ...
    let last = wire.len() - 1;
    wire[last] ^= 0x01;
    assert!(prot.decrypt_long(&mut wire, PN_OFFSET).is_err());
}
